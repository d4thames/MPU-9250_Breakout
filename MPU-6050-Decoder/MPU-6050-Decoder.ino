...
{      
  ...
  // Clear the sleep bit to wake up the MPU6050
  MPU6050_write_reg(MPU6050_PWR_MGMT_1, 0);
}

int filtered_angle_x, filtered_angle_y, filtered_angle_z;
#define A 0.96
#define DELTA 0.04/131
...
void loop()
{
  ...
  filtered_angle_x = (A * (filtered_angle_x + DELTA * accel_t_gyro.value.x_gyro) + (1-A) * accel_t_gyro.value.x_accel) - x_off;
  filtered_angle_y = (A * (filtered_angle_y + DELTA * accel_t_gyro.value.y_gyro) + (1-A) * accel_t_gyro.value.y_accel) - y_off;
  filtered_angle_z = (A * (filtered_angle_z + DELTA * accel_t_gyro.value.z_gyro) + (1-A) * accel_t_gyro.value.z_accel) - z_off;

  if (first) {
    first = 0;
    x_off = filtered_angle_x;
    y_off = filtered_angle_y;
    z_off = filtered_angle_z;
  }

  Serial.print(F("x,y,z : "));
  Serial.print(filtered_angle_x/100, DEC);
  Serial.print(F(", "));
  Serial.print(filtered_angle_y/100, DEC);
  Serial.print(F(", "));
  Serial.print(filtered_angle_z/100, DEC);
  Serial.println(F(""));
}
